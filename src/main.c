...
int main(int argc, char *argv[]) {
  ...
  if (!(argc == 2 || argc == 3 || argc == 4)) {
    ...
    for (int i = 0; i < 64; i++) {
      ...
      if (t1.type == Exit && t2.type == Intydzr) {
      ...
      } else if (t1.type == definuj && t2.type == string &&
                 t3.type == rovnitko) {
        ...
        if (t3.type == strednik) {
          fprintf(textsec, "    dec qword [%s]\n", t2.strvalue);
                 ...
                 ((t2.type == string && t3.type == string) ||
                  (t2.type == string && t3.type == Intydzr) ||
                  (t2.type == Intydzr && t3.type == string) ||
                  (t2.type == Intydzr && t3.type == Intydzr))) {
        Token t4 = next_token(file);
        if (t4.type == strednik) {
          if (t2.type == string) {
            if (t3.type == string) {
      ...
      } else if (t1.type == isequal &&
                 ((t2.type == string || t2.type == Intydzr) &&
                  (t3.type == string || t3.type == Intydzr))) {
        Token t4 = next_token(file);
        Token t5 = next_token(file);
        if (t5.type == strednik) {
          if (t4.type == string) {
      ...
      } else if (t1.type == vyblejdosouboru && t2.type == string &&
                 t3.type == string) {
        Token t4 = next_token(file);
        if (t4.type == strednik) {
          fprintf(textsec,
          ...
          if (t4.type == string) {
            if (t3.type == string) {
              fprintf(textsec, "    cmp %s, %s\n    je %s\n", t2.strvalue,
                      t3.strvalue, t4.strvalue);
            } else {
              fprintf(textsec, "    cmp %s, %i\n    je %s\n", t2.strvalue,
                      t3.value, t4.strvalue);
            }
          } else
            end(1);
        } else
      ...
      } else if (t1.type == movfprint && t2.type == string &&
                 t3.type == string) {
        Token t4 = next_token(file);
        if (t4.type == strednik) {
          fprintf(textsec,
      ...
      } else if (t1.type == getfsize && t2.type == string && t3.type == string) {
        Token t4 = next_token(file);
        if (t4.type == strednik) {
            fprintf(textsec, "    mov rax, 5\n     mov rdi, [rel %s]\n    sub rsp, 144\n    mov rsi, rsp\n    syscall\n    mov rax, [rsp+48]\n    mov [rel %s], rax\n    add rsp, 144\n", t2.strvalue, t3.strvalue);
        } else
          end(0); 
      }
    }
